...
void Engine::init_instance() {
    ...
    InstanceExtensionsFeatures::get_layer_names(&layer_count, nullptr,
                                                V_LAYERS);
    const char *layer_names[layer_count];
    InstanceExtensionsFeatures::get_layer_names(&layer_count, layer_names,
                                                V_LAYERS);
    ...
    InstanceExtensionsFeatures::get_ext_names(&other_ext_count, nullptr,
                                              V_LAYERS);
    const char *other_exts[other_ext_count];
    InstanceExtensionsFeatures::get_ext_names(&other_ext_count, other_exts,
                                              V_LAYERS);
...
void Engine::init_surface() {
    auto check =
        glfwCreateWindowSurface(vk_instance, window->window, NULL, &vk_surface);
...
void Engine::choose_device() {
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        vkGetPhysicalDeviceQueueFamilyProperties(devices[i], &queue_prop_count,
                                                 nullptr);
        ...
        for (uint32_t j = 0; j < queue_prop_count; ++j) {
            ...
            if (!present) {
                vkGetPhysicalDeviceSurfaceSupportKHR(devices[i], j, vk_surface,
                                                     &present);
...
VkResult Engine::alloc_buffer(size_t size, VkBufferUsageFlags usage,
    ...
    auto check =
        vmaCreateBuffer(vma_allocator, &bufCreateInfo, &allocCreateInfo,
                        &buf->buf, &buf->alloc, &buf->alloc_info);
...
void Engine::alloc_staging_buf(size_t size, void *data) {
    ...
    auto check = vmaCreateBuffer(vma_allocator, &bufCreateInfo,
                                 &allocCreateInfo, &staging_buf.buf,
...
void Engine::alloc_vert_buf(size_t size) {
    ...
    auto check =
        vmaCreateBuffer(vma_allocator, &bufCreateInfo, &allocCreateInfo,
                        &vert_buf.buf, &vert_buf.alloc, &vert_buf.alloc_info);
...
void Engine::alloc_ubos(size_t size) {
    ...
    for (int i = 0; i < MAX_FRAME_COUNT; ++i) {
        ...
        auto check =
            vmaCreateBuffer(vma_allocator, &bufferInfo, &allocInfo,
                            &ubos[i].buf, &ubos[i].alloc, &ubos[i].alloc_info);
        DEBUG_OBJ_CREATION(vmaCreateBuffer, check);
    }
}
...
void Engine::get_swapchain_images() {
    ...
    vkGetSwapchainImagesKHR(vk_device, vk_swapchain, &image_count,
...
void Engine::get_swapchain_image_views() {
    ...
    for (uint32_t i = 0; i < swapchain_image_views.length; ++i) {
        ...
        auto check = vkCreateImageView(vk_device, &info, nullptr,
...
void Engine::init_renderpass() {
    ...
    auto check = vkCreateRenderPass(vk_device, &renderpass_info, nullptr,
                                    &vk_renderpass);
...
void Engine::init_framebuffers() {
    ...
    for (uint32_t i = 0; i < swapchain_image_views.length; ++i) {
        ...
        auto check =
            vkCreateFramebuffer(vk_device, &info, nullptr, &vk_framebuffers[i]);
...
void Engine::init_desc_pool() {
    ...
    auto check =
        vkCreateDescriptorPool(vk_device, &pool_info, nullptr, &desc_pool);
...
void Engine::init_desc_set_layout() {
    ...
    auto check_layouts = vkCreateDescriptorSetLayout(
        vk_device, &layout_info, nullptr, &vk_desc_set_layout);
...
void Engine::init_pipeline() {
    VkShaderModule vertex_module =
        create_shader_module("shaders/triangle3.vert.spv");
    VkShaderModule fragment_module =
        create_shader_module("shaders/triangle3.frag.spv");
    ...
    auto check_layout =
        vkCreatePipelineLayout(vk_device, &layout_info, nullptr, &vk_layout);
...
void Engine::init_command() {
    ...
    for (uint32_t i = 0; i < 2; ++i) {
        ...
        auto check_pool = vkCreateCommandPool(vk_device, &pool_info, nullptr,
                                              &vk_commandpools[i]);
...
uint32_t Engine::allocate_commandbuffers(uint32_t pool_index,
    ...
    auto check = vkAllocateCommandBuffers(
        vk_device, &info, vk_commandbuffers.data + vk_commandbuffers.length);
...
void Engine::record_command_buffer(VkCommandBuffer cmd, uint32_t image_index) {
    ...
    vkCmdBindIndexBuffer(cmd, vert_buf.buf, sizeof(Vertex) * 4,
    ...
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, vk_layout, 0,
                            1, &desc_sets[current_frame], 0, nullptr);
...
void Engine::record_and_submit_cpy(size_t size, size_t index_offset) {
    ...
    copy_region.size =
        sizeof(Vertex) * vertex_count + sizeof(Index) * index_count;
...
uint32_t Engine::create_semaphores(uint32_t count, bool binary) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        auto check =
            vkCreateSemaphore(vk_device, &info, nullptr,
                              vk_semaphores.data + vk_semaphores.length);
...
uint32_t Engine::create_fences(uint32_t count, bool signalled) {
    ...
    for (int i = 0; i < count; ++i) {
        VkFenceCreateFlags flags =
            signalled ? VK_FENCE_CREATE_SIGNALED_BIT : 0x0;
        VkFenceCreateInfo info = {
            .sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO,
            .flags = flags,
        };
        auto check = vkCreateFence(vk_device, &info, nullptr,
                                   vk_fences.data + vk_fences.length);
...
void Engine::draw_frame(uint32_t *frame_index) {
    ...
    VkPipelineStageFlags output_stage =
    ...
    auto check_graphics_submit = vkQueueSubmit(
        vk_graphics_queue, 1, &graphics_submit_info, render_done_fence);
...
void Engine::init_debug() {
    VkDebugUtilsMessengerCreateInfoEXT debug_create_info;
    populate_debug_create_info(&debug_create_info);
    VkResult check = vkCreateDebugUtilsMessengerEXT(
        vk_instance, &debug_create_info, nullptr, &debug_messenger);

    DEBUG_ABORT(check == VK_SUCCESS,
                "FAILED TO INIT DEBUG MESSENGER (Aborting)...\n");
}
void Engine::kill_debug() {
    vkDestroyDebugUtilsMessengerEXT(vk_instance, debug_messenger, nullptr);
}

void Engine::populate_debug_create_info(
