...
bool parseSpv(uint32_t *data, size_t dataSize) {
    ...
    while (wordIndex < wordCount) {
        ...
        case (SpvOpTypePointer): {
            ...
            Id &id = ids[idIndex];
            id.opCode = opCode;
            // id.storageClass = (SpvStorageClass)words[wordIndex+2]; <- I think
            // this is unnessecary as the type itself will set the type when i
            // get to that byte but im not sure.
        ...
        case (SpvOpConstant): {
            ...
            id.opCode = opCode;
            id.typeIndex = data[wordIndex + 1];
            id.quiv = data[wordIndex + 3]; // NOTE: this assumes 32bit wide max
                                           // on all constants...
            break;
        }
        case (SpvOpTypeArray): {
        ...
        case (SpvOpTypeStruct): {
            ...
            if (wordCount > 2)
                for (uint32_t memberIndex = 0;
                     memberIndex < (size_t)(wordCount - 2); ++memberIndex) {
                    id.members[memberIndex].typeIndex =
                        data[wordIndex + memberIndex + 2];
                }
    ...
    // pretty sure that this is fine (regarding the string names that are
    // pointers to spvBytes) as all of the information should be uploaded to
    // Vulkan through the creation of the DescriptorSetLayouts, but I am not
    // certain. So check this when fully implemented... They allocate the string
    // buffer for the names and the pointer for the data outside of their
    // function, and the pointer that they also give is a u32
    return true;
}
